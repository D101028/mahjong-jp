"""
    Pai, Furo, Yama, Mentsu, Toitsu, Tehai class
    Param class 
"""
import random
...
class Pai:
    ...
    @staticmethod
    def strict_remove(pai_list: list, pai):
...
class Toitsu:
    def __init__(self, pai: Pai = None, pai_list: list[Pai] = None):
...
class Furo:
    def __init__(self, type: str, pai_tuple: tuple[Pai], minpai_pai: Pai, be_minpai_player_id: Union[int, None]):
        self.type: str = type # lang.koutsu, lang.shuntsu, lang.ankan, lang.kakan, lang.minkan
        self.pai_tuple: tuple[Pai] = pai_tuple
        self.minpai_pai: Union[Pai, None] = minpai_pai # None if is ankan, if kakan -> koutsu be minpai player
        self.be_minpai_player_id: Union[int, None] = be_minpai_player_id # None if is ankan, if kakan -> koutsu be minpai player
...
class AgariComb:
    ...
    def __init__(self, 
                 ...
                 hoora_type: Union[str, None] = None, 
                 akadora_list: list[Pai] = None): 
    ...
    def __str__(self):
        output = ""
        output += self.hoora_type + ": "
        for t in self.toitsu_list:
            output += "(" + ",".join([p.name for p in t.pai_list]) + ")"
        output += " | "
        for m in self.mentsu_list:
            output += "(" + ",".join([p.name for p in m.pai_list]) + ")"
        output += " | "
        output += "(" + ",".join([p.name for p in self.tanhai_list]) + ")"
        return output
...
class TehaiComb:
    ...
    def __str__(self):
        ...
        for f in self.furo_list:
            output += "(" + ",".join([p.name for p in f.pai_tuple]) + ")"
        output += " | "
        output += "(" + ",".join([p.name for p in self.waiting_comb]) + ")"
        output += "；聽：" + self.tenpai.name
        return output
...
def create_mentsu_list(pai_list: list[Pai], first_koutsu_deny: bool = False)  -> Union[list[list[Mentsu]], None]:
...
def get_agari_comb_list(pai_list: list[Pai]) -> list[AgariComb]:
    ...
    for x in double:
        ...
        if len(a1) == 0:
            ...
            result.append(comb)
            break
        for l in create_mentsu_list(a1):
...
class Tehai:
    ...
    def get_tenpais(self) -> list[Pai]:
        self.tenpai_list = get_tenpai_list(self.pai_list)
        return self.tenpai_list

    def get_tehai_comb_list(self) -> list[TehaiComb]:
        if len(self.tenpai_list) == 0:
            self.get_tenpais()
            if len(self.tenpai_list) == 0: # no ten
                self.tehai_comb_list = []
                return self.tehai_comb_list

        result = []
        for p in self.tenpai_list:
            # 分析進 p 的所有面搭組合
...
class Han:
    def __init__(self, name: Union[Yaku, str], is_menchin: bool, dora_hansuu: int = None):
        self.name: str
        self.hansuu: int
...
def get_tensuu(hansuu: int, fusuu: int, is_yakuman: bool, param: Param) -> tuple[int, tuple[int]]:
    basic_tensuu: int
...
def get_yaku_list(tehai_comb: TehaiComb, param: Param) -> list[Yaku]:
    ...
    if is_menchin: # 門清
        ...
        if all(tehai_comb.tenpai_type == lang.ryanmenmachi, # 兩面聽
    ...
    if len(shuntsu_list) >= 3:
        ...
        for m in shuntsu_list:
            ...
            if temp_dict.get(number_string) is not None:
                if pai_type not in temp_dict[number_string]:
                    temp_dict[number_string].append(pai_type)
            else:
    ...
    if len(shuntsu_list) >= 3:
        ...
        for m in shuntsu_list:
            temp_dict[m.pai_list[0].type].append("".join(sorted([str(p.number) for p in m.pai_list])))
        for key, value in temp_dict.items():
            if all("123" in value, "456" in value, "789" in value):
